...
const EnhancedInterview = () => {
  ...
  const initializeSession = async () => {
    try {
      setError("");
      console.log(
        "🔧 Starting initialization with DOM elements already rendered..."
      );
      ...
      try {
        ...
        throw new Error(
          `Event logging initialization failed: ${error.message}`
        );
      }
    ...
    try {
      ...
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/interviews/${sessionId}`,
          {
            timeout: 5000,
          }
        );
  ...
  const initializeSocket = () => {
    try {
      ...
      newSocket.on("connect_error", (error) => {
        console.warn(
          "⚠️ Socket connection error (continuing without real-time features):",
          error.message
        );
      ...
      console.warn(
        "Socket initialization failed, continuing without real-time features:",
        error
      );
  ...
  const initializeDetectionServices = async () => {
    try {
      ...
      if (
        ...
        throw new Error(
          `DOM elements not available after waiting. Available: video=${!!videoRef.current}, focusCanvas=${!!focusCanvasRef.current}, objectCanvas=${!!objectCanvasRef.current}`
        );
      }
      ...
      while (
        (!focusCanvasRef.current || !objectCanvasRef.current) &&
        canvasAttempts < maxCanvasAttempts
      ) {
        await new Promise((resolve) => setTimeout(resolve, 100));
        canvasAttempts++;
      }
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          response.data.integrityScore !== undefined
        ) {
          setInterview((prev) => ({
            ...prev,
  ...
  const stopRecording = async () => {
    try {
      ...
      if (recordingData) {
        await recordingServiceRef.current.saveRecording(
    ...
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        ...
        <div style={styles.statusGrid}>
          <div style={styles.status}>⏱️ {formatTime(elapsedTime)}</div>
          <div
            style={{
              ...styles.status,
