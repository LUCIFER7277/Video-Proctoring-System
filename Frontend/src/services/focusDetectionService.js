// // Use dynamic imports to handle MediaPipe loading issues
// let FaceDetection, Camera;

// // Function to load MediaPipe modules
...
//       console.log('Face detection module:', faceDetectionModule);
...
//     // Try fallback: check if modules are already available globally
//     if (window.FaceDetection && window.Camera) {
//       FaceDetection = window.FaceDetection;
//       Camera = window.Camera;
//       console.log('Using globally available MediaPipe modules');
//       return true;
//     }
...
const loadMediaPipe = async () => {
  try {
    ...
    // Fallback if loaded via <script> in index.html
    if (window.FaceDetection && window.Camera) {
      FaceDetection = window.FaceDetection;
      Camera = window.Camera;
      console.log("⚡ Using globally available MediaPipe modules");
...
class FocusDetectionService {
  ...
  async initialize(videoElement, canvasElement) {
    ...
    const mediaPipeLoaded = await loadMediaPipe();
    if (!mediaPipeLoaded) throw new Error("Failed to load MediaPipe modules");
  ...
  onResults(results) {
    ...
    if (results.detections?.length > 0) {
      console.log("Face(s) detected:", results.detections);
    }
  }
  ...
  destroy() {
    ...
    this.faceDetection = null;
    this.isInitialized = false;
    console.log("🛑 Focus detection service destroyed");
  }
}
